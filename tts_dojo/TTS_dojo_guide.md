...
0. `PIPER TRAINING RAW OUTPUT`  This pane contains all of the text generated by piper as it trains your model.   I generally recommend making this pane 2 rows tall since most of the information provided here isn't very useful.  There are often many warnings that appear in this pane.   Most of them can be ignored.
   ...
   - The first epoch caches phonemes, normalized audio and mel spectrograms (see `training_folder/cache`), so the first epoch is much slower than the rest. The cache location is configurable via `CACHE_DIR` in `SETTINGS.txt` (default `../training_folder/cache`): pointing it at a fast filesystem (e.g. a dedicated docker volume) can reduce GPU starvation on slow disks (NTFS), at the cost of rebuilding the cache after the container is recreated.
   - **Resuming vs fine-tuning** (`FINE_TUNE_LR` in `SETTINGS.txt`): by default, resuming from a saved checkpoint (`Resume from highest saved checkpoint file` in `run_training.sh`) restores the checkpoint's full training state -- epoch counter, optimizer and its learning-rate scheduler. This is the right choice when continuing the *same* training run. If instead you want to *fine-tune* a finished model on new data (e.g. a checkpoint whose LR scheduler has already decayed), set `FINE_TUNE_LR` (e.g. `FINE_TUNE_LR=0.0002`): the checkpoint's weights are loaded but the optimizer and scheduler start **fresh** at that learning rate (the discriminator uses `lr/2`), so the model can actually learn the new patterns instead of taking tiny steps. With `FINE_TUNE_LR` set, training starts at epoch 0 again -- this is a new run by design; use `starting_checkpoint_override` to pick which weights to fine-tune from. Leave `FINE_TUNE_LR` empty for a true resume.
...
![image](https://github.com/user-attachments/assets/a5c6f3fb-123a-4b1b-a134-409c7544d15b)
   ...
   - piper1-gpl keeps the best checkpoints automatically (top 5 by `val_mel`, top 5 by `val_mos`, plus `last.ckpt`), so the grabber mainly copies them to `voice_checkpoints` for safekeeping and exports them as voices.
   ...
   - Since checkpoint files are so large, by default the checkpoint grabber only saves 1 out of every 10 checkpoint files that piper generates. You can change how often checkpoints are automatically saved by pressing `i` to save them less often and and `d` to save them more often.  You can also turn automatic saving off and on by pressing `t`. 
   - Note: a fraction of the dataset (`VALIDATION_SPLIT` in SETTINGS.txt, default 0.1) is held out for validation, which is what piper1-gpl uses to select the best checkpoints.
4. `CONTROL CONSOLE` 
